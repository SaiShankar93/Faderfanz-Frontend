...
const VenueOwnerRegistration = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.venueDetails.venueName.trim())
      errors["venueDetails.venueName"] = "Venue name is required";
    if (!formData.venueDetails.address.trim())
    ...
    if (!formData.contactInfo.password.trim()) {
      errors["contactInfo.password"] = "Password is required";
    } else if (formData.contactInfo.password.length < 6) {
      errors["contactInfo.password"] =
    ...
    if (hasMenuItems) {
      formData.menuProducts.forEach((product, index) => {
        if (!product.name.trim())
          errors[`product${index}Name`] = "Product name is required";
        if (!product.price.trim())
          errors[`product${index}Price`] = "Price is required";
        if (!product.image)
          errors[`product${index}Image`] = "Product image is required";
      });
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-400">
                Upload images Showcasing your event venue
              </p>
              ...
              {hasMenuItems && (
                <div className="space-y-6">
                  <h2 className="text-xl font-semibold">
                    Upload your menu product
                  </h2>
                  {formData.menuProducts.map((product, index) => (
                    <div
                      key={product.id}
                      className="relative space-y-4 p-4 border border-gray-700 rounded-lg bg-[#1A1B23]/80"
                    >
                            ...
                            >
                              {previews.menuImages[product.id] ? (
                                ...
                                <div className="text-center">
                                  ...
                                  />
                                  <span className="text-xs text-[#00FFB3]">
                                    Choose image
                                  </span>
                                </div>
                              )}
